...
func (s *OssBucketSuite) TestSignURLWithSignV4(c *C) {
	endpoint = "oss-cn-chengdu-acdr-ut-1.aliyuncs.com"
	envRegion = "cn-chengdu-acdr-ut-1"
	...
	if stsaccessID == "" || stsaccessKey == "" || stsaccessToken == "" {
		err = fmt.Errorf("sts ak/sk/token can be empty")
		c.Assert(err, IsNil)
	}
	client, err = New(endpoint, stsaccessID, stsaccessKey, SecurityToken(stsaccessToken), Region(envRegion), AuthVersion(AuthV4))
	c.Assert(err, IsNil)
	bucketName = bucketNamePrefix + RandLowStr(10)
	err = client.CreateBucket(bucketName)
	c.Assert(err, IsNil)

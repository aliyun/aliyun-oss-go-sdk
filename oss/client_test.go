...
func (s *OssClientSuite) TestOssClientDo(c *C) {
	client, err := New(endpoint, accessID, accessKey)
    ...
    params["policy"] = nil
    // Test Get Wrong Case
    _, err = client.Do("GET", testBucketName, params, nil, nil)
    c.Assert(err ,NotNil)

    // Test Put Wrong Case
    buffer1 := new(bytes.Buffer)
    buffer1.Write([]byte("test"))

	contentType := http.DetectContentType(buffer1.Bytes())
	headers := map[string]string{}
	headers[HTTPHeaderContentType] = contentType

    _, err = client.Do("PUT", testBucketName, params, headers, buffer1)
    c.Assert(err ,NotNil)

    // Test Put Right Case
    type StStatement struct{
        Action    []string
        Effect    string
        Resource  []string
    }
    ...
    var stStatment StStatement
    stStatment.Action   = []string{"*"}
    stStatment.Effect   = "Allow"
    stStatment.Resource = []string{"acs:oss:*:*:*/*"}

    var test_policy Policy
    test_policy.Version = "1"
    test_policy.Statement = append(test_policy.Statement, stStatment)

    buffer2 := new(bytes.Buffer)
    json_data, err := json.Marshal(test_policy)
    c.Assert(err, IsNil)
    buffer2.Write(json_data)

	contentType = http.DetectContentType(buffer2.Bytes())
	headers[HTTPHeaderContentType] = contentType

    _, err = client.Do("PUT", testBucketName, params, headers, buffer2)
    c.Assert(err ,IsNil)

    rawResp, err := client.Do("GET", testBucketName, params, nil, nil)
    var respPolicy Policy
    err = json.Unmarshal(rawResp, &respPolicy)
    c.Assert(err, IsNil)
    c.Assert(respPolicy.Version, Equals, "1")
    c.Assert(len(respPolicy.Statement), Equals, 1)
    c.Assert(respPolicy.Statement[0].Action[0], Equals, "*")
    c.Assert(respPolicy.Statement[0].Effect, Equals, "Allow")

    _, err = client.Do("DELETE", testBucketName, params, nil, nil)
    c.Assert(err, IsNil)

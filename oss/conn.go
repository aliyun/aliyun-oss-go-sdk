...
func (conn Conn) doRequest(method string, uri *url.URL, canonicalizedResource string, headers map[string]string,
	...
	if conn.config.IsHTTPDebug && conn.config.HTTPDebugWriter != nil {
		DebugOutputHttpReq(conn.config.HTTPDebugWriter, req, startT, reqID, signStr)
	}
	...
	if conn.config.IsHTTPDebug && conn.config.HTTPDebugWriter != nil {
		//print out http resp
		DebugOutputHttpResp(conn.config.HTTPDebugWriter, resp, startT, reqID)
	}
...
func DebugOutputHttpReq(writer io.Writer, req *http.Request, startT time.Time, reqID string, signStr string) {
	...
	logBuffer.WriteTo(writer)
}

func DebugOutputHttpResp(writer io.Writer, resp *http.Response, startT time.Time, reqID string) {
	endT := time.Now()
